...
class VoicePipelineAgent(utils.EventEmitter[EventTypes]):
    ...
    def __init__(
        ...
        *,
        ...
        node_type_after_transition: dict = {},  # transition_id: node_type(conversation, function)
        prompt_type_after_transition: dict = {},  # transition_id: prompt_type(dynamic, static, none)
        # VOXAI_NATIVE_CODE: FOR FUNCTION CALL STREAMING
        ###########################################################
    ) -> None:
        """
    ...
    def _synthesize_agent_speech(
        ...
        async def _llm_stream_to_str_generator(
            ...
            try:
                ...
                transition_id = None
                node_type: Literal["conversation", "function"] = None
                prompt_type: Literal["dynamic", "static", "none"] = None
                # VOXAI_NATIVE_CODE: FOR FUNCTION CALL STREAMING
                ###########################################################
    ...
    async def trigger_followup_response(self):
        ...
        # Use the before_llm_cb directly
        llm_stream = self._opts.before_llm_cb(self, copied_ctx)
        if asyncio.iscoroutine(llm_stream):
            llm_stream = await llm_stream
