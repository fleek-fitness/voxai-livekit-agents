...
import contextvars
import time
import re  # VOXAI_NATIVE_CODE: FOR FUNCTION CALL STREAMING
...
# Import your existing submodules exactly as in the original VoicePipelineAgent
...
from ..llm import (
    ...
    FunctionContext,
    LLMStream,
    FunctionCallInfo,  # VOXAI_NATIVE_CODE
    FunctionInfo,  # VOXAI_NATIVE_CODE
    CalledFunction,  # VOXAI_NATIVE_CODE
)
...
BeforeLLMCallback = Callable[
    ["FlowVoicePipelineAgent", ChatContext],
    Union[
        Optional[LLMStream],
...
BeforeTTSCallback = Callable[
    ["FlowVoicePipelineAgent", Union[str, AsyncIterable[str]]],
    SpeechSource,
]
...
class AgentCallContext:
    """
    Same as in VoicePipelineAgent, we copy it here but if you have it in a shared place,
    just import it.
    """

    _assistant: "FlowVoicePipelineAgent"
    _llm_stream: LLMStream

    def __init__(
        self, assistant: "FlowVoicePipelineAgent", llm_stream: LLMStream
    ) -> None:
    ...
    @property
    def agent(self) -> "FlowVoicePipelineAgent":
...
def _default_before_llm_cb(
    agent: "FlowVoicePipelineAgent", chat_ctx: ChatContext
) -> LLMStream:
...
def _default_before_tts_cb(
    agent: "FlowVoicePipelineAgent", text: str | AsyncIterable[str]
...
class FlowVoicePipelineAgent(utils.EventEmitter[EventTypes]):
    """
    A pipeline agent (VAD + STT + LLM + TTS) with modifications for partial
    function call streaming, labeled by # VOXAI_NATIVE_CODE.
    """
    ...
    def __init__(
        ...
        *,
        ...
        node_type_after_transition: dict = {},
        prompt_type_after_transition: dict = {},
    ...
    # -- The rest of the methods from the original VoicePipelineAgent remain the same,
    #    except for the # VOXAI_NATIVE_CODE lines inside them. --
    ...
    def start(
        ...
        def _on_stt_metrics(stt_metrics: metrics.STTMetrics) -> None:
            self.emit(
                "metrics_collected",
                metrics.PipelineSTTMetrics(**stt_metrics.__dict__),
            )
        ...
        if participant is not None:
            ...
            for participant_obj in self._room.remote_participants.values():
                self._link_participant(participant_obj.identity)
                break
    ...
    async def say(
        ...
        *,
        ...
        if add_to_chat_ctx:
            try:
                ...
                if isinstance(source, LLMStream):
                    logger.warning(
                        "LLMStream will be ignored for function call context"
                    )
                elif isinstance(source, AsyncIterable):
                    from ..utils.aio.itertools import tee

                    source, fnc_source = tee(source, 2)  # type: ignore
                else:
                    fnc_source = source
        ...
        if call_ctx is not None and fnc_source is not None:
            ...
            logger.debug(
                "added speech to function call chat context", extra={"text": text}
            )
    ...
    def _link_participant(self, identity: str) -> None:
        ...
        def _on_final_transcript(ev: stt.SpeechEvent) -> None:
            ...
            if not new_transcript:
                return
            logger.debug(
                "received user transcript", extra={"user_transcript": new_transcript}
            )
            self._last_final_transcript_time = time.perf_counter()
            self._transcribed_text += (
                (" " + new_transcript) if self._transcribed_text else new_transcript
            )

            if self._opts.preemptive_synthesis:
                if (self._playing_speech is None) or (
                    self._playing_speech.allow_interruptions
                ):
    ...
    async def _play_speech(self, speech_handle: SpeechHandle) -> None:
        ...
        def _commit_user_question_if_needed() -> None:
            ...
            if (
                ...
                and play_handle.time_played < self.MIN_TIME_PLAYED_FOR_COMMIT
    ...
    def _synthesize_agent_speech(
        ...
        async def _llm_stream_to_str_generator(
            ...
            """
            Slightly modified from the original to handle partial streaming for
            execute_transition calls. # VOXAI_NATIVE_CODE: FOR FUNCTION CALL STREAMING
            """
            ...
            last_yielded = ""
            transition_id = None
            node_type: Literal["conversation", "function"] | None = None
            prompt_type: Literal["dynamic", "static", "none"] | None = None
            # END VOXAI_NATIVE_CODE
                        ...
                        if chunk.choices[0].delta.tool_calls:
                            ...
                            if tool_calls:
                                ...
                                if function_name is None and tc.function.name:
                                    logger.info(
                                        "EXECUTE_TRANSITION_STREAMING ACTIVATED"
                                    )
                                ...
                                # Attempt to parse out "transition_id":
                                complete_pattern = (
        ...
        if isinstance(original_source, AsyncIterable):
            from ..utils.aio.itertools import tee

            original_source, transcript_source = tee(original_source, 2)

        tts_source = self._opts.before_tts_cb(self, original_source)
    ...
    def _validate_reply_if_possible(self) -> None:
        ...
        logger.debug(
            ...
            extra={
                "speech_id": (
                    self._pending_agent_reply.id if self._pending_agent_reply else None
                ),
    ...
    async def execute_function_directly(
        ...
        try:
            ...
            if speak_result and result is not None:
                ...
                tool_call_msg = ChatMessage.create_tool_calls([call_info])
...
class _DeferredReplyValidation:
    """
    Identical to VoicePipelineAgent’s internal class, just copied here.
    """
    ...
    def validating(self) -> bool:
        return (self._validating_task is not None) and (
            not self._validating_task.done()
        )
    ...
    def on_human_start_of_speech(self, ev: vad.VADEvent) -> None:
        ...
        if self.validating and self._validating_task is not None:
